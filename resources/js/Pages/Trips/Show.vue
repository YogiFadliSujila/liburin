...
const copyJoinCode = () => {
    if (props.trip.join_code) {
        navigator.clipboard.writeText(props.trip.join_code);
...
const regenerateCode = () => {
    if (confirm('Apakah Anda yakin ingin membuat kode baru? Kode lama tidak akan bisa digunakan lagi.')) {
        router.post(route('trips.regenerate-code', props.trip.id));
    }
};
    ...
    <AuthenticatedLayout :full-width="true">
        ...
        <div class="min-h-screen relative">
                ...
                <div v-show="activeTab === 'members'" class="space-y-6 px-4 sm:px-6 lg:px-8">
                    ...
                    <!-- Join Code Share Card (Admin Only) -->
                    <div v-if="isAdmin && trip.join_code" class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl p-5 text-white shadow-lg">
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                <h4 class="font-bold text-lg">Kode Join Trip</h4>
                                <p class="text-indigo-100 text-sm">Bagikan kode ini untuk mengundang anggota baru</p>
                            </div>
                            <button
                                @click="regenerateCode"
                                class="text-indigo-100 hover:text-white transition p-2 rounded-lg hover:bg-white/10"
                        ...
                        <div class="flex items-center gap-3">
                            <div class="flex-1 bg-white/20 rounded-xl px-4 py-3 backdrop-blur-sm">
                                <span class="text-2xl font-mono tracking-widest font-bold">{{ trip.join_code }}</span>
                            </div>
                            <button
                                @click="copyJoinCode"
                                class="flex items-center gap-2 bg-white text-indigo-600 px-4 py-3 rounded-xl font-medium hover:bg-indigo-50 transition shadow"
                            >
